(function(window, angular, $) {
    'use strict';
    angular.module('FileManagerApp').factory('item', ['$http', '$q', '$translate', 'fileManagerConfig', 'chmod', 'drive', function($http, $q, $translate, fileManagerConfig, Chmod, drive) {
        ...
        Item.prototype.download = function() {
            ...
            // var deferred = $q.defer();
            // // var data = {params: {
            // //     id: self.tempModel._id,
            // //     path: self.tempModel.fullPath()
            // // }};
            // self.inprocess = true;
            // self.error = '';
            // var req = fileDownloader.download(self.tempModel.id).then(function(data) {
            //     self.deferredHandler(data, deferred);
            //     self.inprocess = false;
            // }, function(data) {
        ...
        Item.prototype.remove = function() {
            ...
            self.inprocess = true;
            self.error = '';
            $http.delete(fileManagerConfig.removeUrl + self.tempModel.id).success(function(data) {
                self.deferredHandler(data, deferred);
            }).error(function(data) {
                self.deferredHandler(data, deferred, $translate.instant('error_deleting'));
            })['finally'](function() {
